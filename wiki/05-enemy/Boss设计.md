...
- **刀体渲染**（测试小修 #005）：Boss 刀体由渲染管线 `RenderLayer.Blades` 层统一绘制（与玩家/小怪刀体同层），由 `BladeRenderer.drawBlade` 按品质色→白渐变 + 青色拖尾渲染；`BattleState` 渲染层遍历需同时覆盖 `playerBlades` / `enemies.blades` / `boss.blades` 三类，漏任一类会导致武器不可见
- Boss 刀体动量高于玩家基准（见总览表），主动拼刀胜率被压低（约 30-40%）
- Boss 特定技能有**拼刀窗口**（出刀前摇期间玩家拼刀胜率 +0.15），鼓励抓时机
- **范围技能预警**（测试小修 #004）：横扫/重击等圆形判定技能前摇期间，以 Boss 为圆心绘制攻击范围淡圈（淡橙虚线，半径=判定半径），供玩家预判走位；冲锋等直线技能不画圈
