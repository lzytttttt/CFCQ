...
| v0.11 | 2026-08-19 | 开发里程碑 7（Rogue 流程）落地：关卡设计总览.md §5 新增「实现细节」小节——房间过渡（清怪开门走入门区）、商店交互（走近按 E）、POI 交互（宝箱/休息/事件参数）、战斗房波次结构、种子复现机制。随机关卡生成（保底商店/精英房）、商店经济（金币掉落+购买）、6 关推进、localStorage 图鉴存档运行时全链路验证：完整通关第 1 关进入第 2 关（269 单测通过） |
| v0.12 | 2026-08-19 | 开发里程碑 8（Boss 战与剧情）落地：主线剧情脚本.md §1 补对话推进交互（E 键先补全后翻页）；Boss设计.md §4 标注 M8 精简实现范围（每 Boss 2-3 代表技+阶段机/拼刀窗口/打断全量，复杂场地机制为打磨项）。6 个多阶段 Boss、6 章五段剧情对话、双结局（持家传刀觉醒）、Boss 掉刀收集与背包刀具切换运行时全链路验证：完整 Boss 战（战前对话→阶段切换→击败→掉刀→通关进第 2 关，285 单测通过） |
| v0.13 | 2026-08-19 | 开发里程碑 9（性能与平衡打磨，最终里程碑）落地：主菜单（水墨玄黑+鎏金标题+旋转刀光动效）、死亡/胜利结算画面（Build 摘要：击杀/拼刀胜率/最高连击/到达关卡）、破刀慢镜头（0.3s 白闪+全场暗化叠加闪光队列）、多刀伤害递减（平衡性分析 §6 预案：第2刀80%/第3刀60%）、破军 4 件连 2 次触发刀势如虹、装备聚合脏标记缓存（每帧多次遍历→变化时重算）。**全流程通关验证：6 关自动通关→觉醒结局 B（藏锋·无名）、刀法 Lv20 满级 19 次升级（精确吻合升级曲线 §6 设计目标）、165 FPS、生产构建 47KB gzip**。游戏 v0.1.5 全量交付 |
